    ...
    <body>
        <div class="container">
            ...
            <div class="order-info">
                <div>
                    <p>Order Code: <a href="#">${orderDetails[0].orderID.orderID}</a></p>
                    <p>${orderDetails[0].orderID.orderDate} </p>
                </div>
                <span class="status">
                    <c:choose>
                        <c:when test="${orderDetails[0].orderID.statusID == 1}">Pending</c:when>
                        <c:when test="${orderDetails[0].orderID.statusID == 2}">Approved</c:when>
                        <c:when test="${orderDetails[0].orderID.statusID == 3}">Delivering</c:when>
                        <c:when test="${orderDetails[0].orderID.statusID == 4}">Delivered</c:when>
                        <c:when test="${orderDetails[0].orderID.statusID == 5}">Rejected</c:when>
                        <c:when test="${orderDetails[0].orderID.statusID == 6}">Canceled</c:when>
            ...
            <div class="customer-receiver">
                <div class="info-block">
                    <h3>Customer</h3>
                    <p>${orderDetails[0].account.fullName}</p> <!-- Sử dụng fullname từ Account -->
                    <p>${orderDetails[0].account.phoneNumber}</p>
                </div>
                <div class="info-block">
                    <h3>Place</h3>
                    <p>${orderDetails[0].account.email}</p> <!-- Sử dụng fullname từ Account -->
                    <p>${orderDetails[0].orderID.address}</p>
            ...
            <div class="payment-info">
                ...
                <div class="summary">
                    ...
                    <p>Ship: <span>${shippingFee == 0 ? 'Free' : shippingFee + 'đ'}</span></p>
