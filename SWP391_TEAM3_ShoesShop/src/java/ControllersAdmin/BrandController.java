...
public class BrandController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (keyW != null && !keyW.isEmpty()) {
                      listBrands = listBrandDao.getListBrand(keyW);

        } else { listBrands = listBrandDao.getListBrand(); 
        }

        request.setAttribute("listBrands", listBrands);
        request.setAttribute("searchKeyword", keyW);
        request.getRequestDispatcher("Views/Admin/ListBrand.jsp").forward(request, response);
    }
