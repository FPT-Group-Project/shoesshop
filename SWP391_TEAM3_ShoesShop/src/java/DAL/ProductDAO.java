...
public class ProductDAO extends DBContext {
    ...
    public Product getProductFromProductAdmin(ProductAdmin pa) {
        BrandDAO brd = new BrandDAO();
        Product p = new Product(pa.getProductID(), pa.getProductName(), pa.getDescription(), pa.getPrice(), pa.getBrandID().getBrandID(), pa.getAvatarP());
        return p;
    }
...
//        prd.printAllProducts(list);
    ...
    public String getAvatarById(int productId) {
        String sql = "SELECT avatarP FROM Product WHERE ProductID = ?";
        String avatarP = null;
        ...
        try {
            ...
            if (rs.next()) {
                avatarP = rs.getString("avatarP");
            } else {
                System.out.println("Product doesn't exist");
            }
        } catch (SQLException e) {
            System.out.println("Can't retrieve avatar");
        }
